...
using ToggleDebugOverlayFn = void (*)(CBaseEntity*, DebugOverlayBits_t);

ToggleDebugOverlayFn g_fnAddDebugOverlayBits = nullptr;
...
void CShowTrigger::TransmitTriggers(const std::vector<CHandle<CBaseTrigger>>& vTriggers, bool bTransmit) {
	for (const auto& hTrigger : vTriggers) {
		...
		static DebugOverlayBits_t iTramsmitBits = DebugOverlayBits_t(DebugOverlayBits_t::OVERLAY_NAME_BIT | DebugOverlayBits_t::OVERLAY_BBOX_BIT);
		if (bTransmit) {
			g_fnAddDebugOverlayBits(pTrigger, iTramsmitBits);
		} else {
			g_fnRemoveDebugOverlayBits(pTrigger, iTramsmitBits);
		}
	}
}
